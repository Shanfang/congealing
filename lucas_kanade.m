...
function [u, v] = lucas_kanade(image1, image2, 50)
...
fx = conv2(images1, 0.25 * [-1 1; -1 1]) + conv2(images2, 0.25 * [-1 1; -1 1]);
fy = conv2(images1, 0.25 * [-1 -1; 1 1]) + conv2(images2, 0.25 * [-1 -1; 1 1]);
ft = conv2(images1, 0.25 * ones(2)) + conv2(images2, -0.25 * ones(2));

% Calculate optical flow
window_center = floor(window_size / 2);
image_size = size(images1);
u = zeros(image_size);
v = zeros(image_size);
for i = window_center + 1:image_size(1) - window_center
  for j = window_center + 1:image_size(2) - window_center
    ...
    U = pinv(A' * A) * A' * -ft_window(:); % U is 2-by-1
...
for i = 1:image_size
  if (i == image_size)
    format = [format '%g']; %format it as floating point 
  else
    format = [format '%g,'];
  end
end
...
% Write flow vectors to output file with comma separated value 
file = fopen('output.csv', 'w');
fprintf(file, format, u);
fprintf(file, format, v);
fclose(file);

% Display the result, velocity plot with quiver
figure
axis equal
